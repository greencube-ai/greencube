...
mod tests {
    ...
    fn test_select_model_for_ram_uses_expected_tiers() {
        // <20 GB: phi4-mini, llama3-8b and qwen3.5-9b all have min_ram_gb=0;
        // the loop picks the heaviest (qwen3.5-9b) since it is last in MODELS order.
        assert_eq!(select_model_for_ram(8).id, "qwen3.5-9b");
        assert_eq!(select_model_for_ram(16).id, "qwen3.5-9b");
        assert_eq!(select_model_for_ram(19).id, "qwen3.5-9b");
        assert_eq!(select_model_for_ram(20).id, "qwen3-14b");
        assert_eq!(select_model_for_ram(31).id, "qwen3-14b");
    ...
    fn test_select_model_for_hardware_downshifts_on_battery() {
        // 16 GB base is qwen3.5-9b; on-battery downshift goes to the next lighter
        // entry in MODELS, which is llama3-8b (kept as legacy fallback).
        assert_eq!(
