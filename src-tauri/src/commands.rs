...
fn build_system_block(memories: &[db::Memory]) -> String {
    ...
    let mut system = String::from(
        "You are GreenCube, a helpful AI assistant. You can help with coding, writing, analysis, and any other task. You give clear, direct, and complete answers.",
    );
...
fn spawn_summary_refresh(
    ...
    std::thread::spawn(move || {
        ...
        let n_ctx = ctx_size_for(&loaded_state.path);
...
pub fn send_message_streaming(
    ...
    std::thread::spawn(move || {
        let thread_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            let n_ctx = ctx_size_for(&ls.path);
